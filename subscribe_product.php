<?php
/**
 * @author Walter Omedo - Frontier Optical Networks Limited -- Subscribe Product
 * 2nd June 2017
 */
...
class MySoapClient extends SoapClient {
    public function __doRequest($sRequest, $sLocation, $sAction, $iVersion, $iOneWay = 0) {
        $sRequest = str_replace('ns1', 'sub', $sRequest);
...
//User credentials
  ...
  $product_code =10805625;



function call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$subscriber_no,$product_code,$service) {
  ...
  $xml->startElementNS($method,'ServiceRequest',null);//Service Request
	...
	$xml->startElementNS($method,'Body',null);//Body
		...
		$xml->endElement();
		$xml->startElementNS($method,'ProductCode',null);//ProductCode
		$xml->Text($product_code);
		$xml->endElement();
	$xml->endElement();
 ...
 $ns = 'http://safaricom.co.ke/schemas/partners/LTE_sharing/SubscribeProduct'; //Namespace of the web service.
...
echo call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$subscriber_no,$product_code,$service);
?>
