<?php
/**
 * @author Walter Omedo - Frontier Optical Networks Limited -- Get Subscriber Balance
 * 31st May 2017
 */
...
//User credentials
  ...
  $partner_code ='1002';
  $cur_time = date('Ymd');
  $action ='getSubscriberBalance';
  $service ='getSubscriberBalance';
  $version ='1.0';
  $method ='get';
...
function call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$params,$subscriber_no,$query_type,$service) {
  ...
  $xml->startElementNS($method,'ServiceRequest',null);//Service Request
	...
	$xml->startElementNS($method,'Body',null);//Body
		...
		$xml->endElement();
		$xml->startElementNS($method,'QueryType',null);//Query Type
		$xml->Text($query_type);
		$xml->endElement();
	$xml->endElement();
  ...
  $ns = 'http://safaricom.co.ke/schemas/partners/LTE_sharing/GetSubscriberBalance'; //Namespace of the web service.
...
echo call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$params,$subscriber_no,$query_type,$service);
?>
