<?php
/**
 * @author Walter Omedo - Frontier Optical Networks Limited -- SIM SWAP
 * 2nd June 2017
 */
...
class MySoapClient extends SoapClient {
    public function __doRequest($sRequest, $sLocation, $sAction, $iVersion, $iOneWay = 0) {
        $sRequest = str_replace('ns1', 'swap', $sRequest);
...
//User credentials
  ...
  $oldiccid = 89254021024081399364;
  $newiccid = 89254029441010331312;
  $remarks ='Swap Lost SIM Card';



function call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$subscriber_no,$oldiccid,$newiccid,$remarks,$service) {
  ...
  $xml->startElementNS($method,'ServiceRequest',null);//Service Request
	...
	$xml->startElementNS($method,'Body',null);//Body
		...
		$xml->endElement();
		$xml->startElementNS($method,'OldICCID',null);//OLD ICCID 
		$xml->Text($oldiccid);
		$xml->endElement();
		$xml->startElementNS($method,'NewICCID',null);//NEW ICCID 
		$xml->Text($newiccid);
		$xml->endElement();
		$xml->startElementNS($method,'Remarks',null);//Remarks
		$xml->Text($remarks);
		$xml->endElement();
	$xml->endElement();
  ...
  $ns = 'http://safaricom.co.ke/schemas/partners/LTE_sharing/SwapSIMCard'; //Namespace of the web service.
...
echo call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$subscriber_no,$oldiccid,$newiccid,$remarks,$service);
?>
