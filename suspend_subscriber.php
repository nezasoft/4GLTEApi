<?php
/**
 * @author Walter Omedo - Frontier Optical Networks Limited -- Suspend Subscriber
 * 2nd June 2017
 */
...
class MySoapClient extends SoapClient {
    public function __doRequest($sRequest, $sLocation, $sAction, $iVersion, $iOneWay = 0) {
        $sRequest = str_replace('ns1', 'sus', $sRequest);
...
//User credentials
  ...
  $partner_code ='1002';
  $cur_time = date('Ymd');
  $action ='suspendResume';
  $service ='suspendResume';
  $version ='1.0';
  $method ='sus';
...
function call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$subscriber_no,$suspend_flag,$service) {
  ...
  $xml->startElementNS($method,'ServiceRequest',null);//Service Request
	...
	$xml->startElementNS($method,'Body',null);//Body
		...
		$xml->endElement();
		$xml->startElementNS($method,'SuspendFlag',null);//Suspend Flag
		$xml->Text($suspend_flag);
		$xml->endElement();
	$xml->endElement();
  ...
  $ns = 'http://safaricom.co.ke/schemas/partners/LTE_sharing/SuspendResume'; //Namespace of the web service.
...
echo call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$subscriber_no,$suspend_flag,$service);
?>
