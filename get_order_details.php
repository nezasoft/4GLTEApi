<?php
/**
 * @author Walter Omedo - Frontier Optical Networks Limited -- Get Order Details
 * 31st May 2017
 */
...
function call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$params,$subscriber_no,$order_no,$service) {
  ...
  $xml->startElementNS($method,'ServiceRequest',null);//Service Request
	...
	$xml->startElementNS($method,'Body',null);//Body
		...
		$xml->endElement();
		$xml->startElementNS($method,'OrderNumber',null);//Order Number
		$xml->Text($order_no);
		$xml->endElement();
	$xml->endElement();
  ...
  $ns = 'http://safaricom.co.ke/schemas/partners/LTE_sharing/GetOrderDetails'; //Namespace of the web service.
...
echo call_soap_wsdl($method,$portaluser,$username,$password,$partner_code,$cur_time,$action,$version,$params,$subscriber_no,$order_no,$service);
?>
